...
from semantic_tokenizer.src.trainers.trainer_utils import visualize_latent_motion_reconstruction
import omegaconf
from glob import glob
import shutil
from collections import defaultdict
from semantic_tokenizer.src.trainers.optimizer import get_optimizer, LinearWarmup_CosineAnnealing
...
class LatentMotionTokenizer_Trainer:
    ...
    def eval_latent_motion_reconstruction(self, visualization_dir):
        ...
        semantic_seq = torch.cat([batch['semantic_initial'], batch['semantic_future']], dim=1)

        self.latent_motion_tokenizer.eval()
        outputs = self.latent_motion_tokenizer(
            cond_pixel_values=semantic_seq[:,0],
            target_pixel_values=semantic_seq[:,1],

...
from flow_tokenizer.src.trainers.trainer_utils import visualize_latent_motion_reconstruction, visualize_latent_motion_reconstruction_flow
import omegaconf
from glob import glob
import shutil
from collections import defaultdict
from flow_tokenizer.src.trainers.optimizer import get_optimizer, LinearWarmup_CosineAnnealing
...
class LatentMotionTokenizer_Trainer:
    ...
    def eval_latent_motion_reconstruction(self, visualization_dir):
        ...
        flow_seq = torch.cat([batch['flow_initial'], batch['flow_future']], dim=1)

        self.latent_motion_tokenizer.eval()
        outputs = self.latent_motion_tokenizer(
            cond_pixel_values=flow_seq[:,0],
            target_pixel_values=flow_seq[:,1],
            cond_pixel_values_rgb=rgb_seq[:,0],
            target_pixel_values_rgb=rgb_seq[:,1],
            return_recons_only=True
        )

        cur_flow_in = outputs["cur_flow_in"].detach().cpu()
        fut_flow_in = outputs["fut_flow_in"].detach().cpu()
        recon_flow = outputs['recons_pixel_values'].detach().cpu()
            
        recons_rgb_future = self.rgb_preprocessor.post_process(outputs["recons_pixel_values_rgb"]).detach().cpu()  # (b, c, h, w)
        ...
        for i in range(orig_rgb_seq.shape[0]):
            visualize_latent_motion_reconstruction_flow(
                initial_flow=cur_flow_in[i],
                next_flow=fut_flow_in[i],
                recons_next_flow=recon_flow[i],
                latent_motion_ids=gt_latent_motion_ids[i],
                path=os.path.join(visualization_dir, f"flow-{self.process_index}-{i}.png")
            )
            ...
            visualize_latent_motion_reconstruction(
                ...
                latent_motion_ids=gt_latent_motion_ids[i],
                path=os.path.join(visualization_dir, f"rgb-{self.process_index}-{i}.png")
            )
    ...
    def calculate_loss(self, batch, train):
        ...
        rgb_seq_rgb = self.rgb_preprocessor(rgb_seq, train=train)
        flow_seq = torch.cat([batch['flow_initial'], batch['flow_future']], dim=1)
        ...
        loss = self.latent_motion_tokenizer(
            cond_pixel_values=flow_seq[:,0],
            target_pixel_values=flow_seq[:,1],
            cond_pixel_values_rgb=rgb_seq_rgb[:,0],
            target_pixel_values_rgb=rgb_seq_rgb[:,1],
        )

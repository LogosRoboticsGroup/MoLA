...
class ConvBlock(nn.Module):
    def __init__(self, in_feature, out_feature):
        ...
        self.conv_block = nn.Sequential(
            nn.Conv2d(
                in_feature, out_feature, kernel_size=3, stride=1, padding=1
            ),
...
class DepthAnythingV2(nn.Module):
    ...
    def forward(self, x):
        ...
        return depth.squeeze(1), depth_feature

    @torch.no_grad()
    def infer_image(self, raw_image, input_size=518):
        image, (h, w) = self.image2tensor(raw_image, input_size)

        depth, depth_feature = self.forward(image)

        depth = F.interpolate(
            depth[:, None], (h, w), mode="bilinear", align_corners=True
        )[0, 0]

        return depth, depth_feature

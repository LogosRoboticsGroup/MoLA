...
from depth_tokenizer.src.trainers.trainer_utils import visualize_latent_motion_reconstruction, visualize_latent_motion_reconstruction_depth
import omegaconf
from glob import glob
import shutil
from collections import defaultdict
from depth_tokenizer.src.trainers.optimizer import get_optimizer, LinearWarmup_CosineAnnealing
...
class LatentMotionTokenizer_Trainer:
    ...
    def eval_latent_motion_reconstruction(self, visualization_dir):
        ...
        outputs = self.latent_motion_tokenizer(
            cond_pixel_values=orig_rgb_seq[:,0],
            target_pixel_values=orig_rgb_seq[:,1],
        ...
        cur_depth_in = outputs["cur_depth_in"].unsqueeze(1).detach().cpu()
        fut_depth_in = outputs["fut_depth_in"].unsqueeze(1).detach().cpu()
        recon_depth = outputs['recons_pixel_values'].detach().cpu()

        for i in range(orig_rgb_seq.shape[0]):
            visualize_latent_motion_reconstruction_depth(
                initial_depth=cur_depth_in[i],
                next_depth=fut_depth_in[i],
                recons_next_depth=recon_depth[i],
                latent_motion_ids=gt_latent_motion_ids[i],
                path=os.path.join(visualization_dir, f"depth-{self.process_index}-{i}.png")
            )

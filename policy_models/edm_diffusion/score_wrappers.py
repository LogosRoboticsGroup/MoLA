...
class GCDenoiser(nn.Module):
    ...
    def loss(self, state, action, goal, noise, sigma, latent_motion_tokens_up, **kwargs):
        """
        ...
        model_output = self.inner_model(state, latent_motion_tokens_up, noised_input * c_in, goal, sigma, **kwargs)
        target = (action - c_skip * noised_input) / c_out
        return (model_output - target).pow(2).flatten(1).mean(), model_output

    def forward(self, latent_motion_tokens_up, state, action, goal, sigma, **kwargs):
        """
        ...
        """
        c_skip, c_out, c_in = [append_dims(x, action.ndim) for x in self.get_scalings(sigma)]
        return self.inner_model(state, latent_motion_tokens_up, action * c_in, goal, sigma, **kwargs) * c_out + action * c_skip

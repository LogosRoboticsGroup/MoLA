...
class VPP_Policy(pl.LightningModule):
    ...
    def __init__(
        ...
        latent_flow_motion_tokenizer = hydra.utils.instantiate(latent_motion_tokenizer_config)
        self.latent_flow_motion_tokenizer = latent_flow_motion_tokenizer
        ...
        latent_semantic_motion_tokenizer = hydra.utils.instantiate(latent_motion_tokenizer_config)
        self.latent_semantic_motion_tokenizer = latent_semantic_motion_tokenizer
    ...
    def configure_optimizers(self):
        ...
        optim_groups = [
            ...
            "weight_decay": self.optimizer_config.transformer_weight_decay},
            {"params": self.latent_flow_motion_tokenizer.parameters(),
            "weight_decay": self.optimizer_config.transformer_weight_decay},
            {"params": self.latent_depth_motion_tokenizer.parameters(),
            "weight_decay": self.optimizer_config.transformer_weight_decay},
            {"params": self.latent_semantic_motion_tokenizer.parameters(),
            "weight_decay": self.optimizer_config.transformer_weight_decay},
    ...
    def training_step(self, dataset_batch: Dict[str, Dict],) -> torch.Tensor:  # type: ignore
        ...
        latent_flow_motion_tokens_up, indices_flow, commit_loss_flow = self.latent_flow_motion_tokenizer(
        ...
        latent_semantic_motion_tokens_up, indices_semantic, commit_loss_semantic = self.latent_semantic_motion_tokenizer(
            cond_pixel_values = cond_pixel_values,
            target_pixel_values = target_pixel_values,
        )
    ...
    def libero_eval_forward(self,image_primary,image_wrist,text):
            ...
            time_pos_emb = (
                self.time_pos_emb.unsqueeze(0).expand(
                    perceptual_features.size(0),
                    -1,
                    -1,
                    -1
                )
            ...
            latent_motion_tokens_up = torch.cat(
                [
                    latent_flow_motion_tokens_up,
                    latent_depth_motion_tokens_up,
                    latent_semantic_motion_tokens_up
                ],
                dim=1
            )

            perceptual_emb['state_images'] = perceptual_emb['state_images'].reshape(
                perceptual_emb['state_images'].size(0),
                -1,
                perceptual_emb['state_images'].size(-1)
            )
    ...
    def on_train_start(self) -> None:
        self.latent_flow_motion_tokenizer.to(dtype=self.dtype)
        self.latent_depth_motion_tokenizer.to(dtype=self.dtype)
        self.latent_semantic_motion_tokenizer.to(dtype=self.dtype)

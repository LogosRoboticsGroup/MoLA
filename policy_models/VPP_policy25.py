...
class VPP_Policy(pl.LightningModule):
    ...
    def __init__(
            ...
            seed: int = 42,
            pretrained_model_path: str = 'stabilityai/stable-video-diffusion-img2vid',
            text_encoder_path: str = 'openai/clip-vit-base-patch32',
        ...
        self.time_pos_emb = nn.Parameter(torch.randn(2, 1, 1024))

        # policy network
        latent_motion_tokenizer_config_path = Path("policy_models/m_former/config/latent_motion_tokenizer.yaml")
        ...
        latent_cotracker_motion_tokenizer = hydra.utils.instantiate(latent_motion_tokenizer_config)
        self.latent_cotracker_motion_tokenizer = latent_cotracker_motion_tokenizer

        latent_depth_motion_tokenizer = hydra.utils.instantiate(latent_motion_tokenizer_config)
        self.latent_depth_motion_tokenizer = latent_depth_motion_tokenizer

        latent_segment_motion_tokenizer = hydra.utils.instantiate(latent_motion_tokenizer_config)
        self.latent_segment_motion_tokenizer = latent_segment_motion_tokenizer
    ...
    def configure_optimizers(self):
        ...
        optim_groups = [
            ...
            "weight_decay": self.optimizer_config.transformer_weight_decay},
            {"params": self.latent_cotracker_motion_tokenizer.parameters(),
            "weight_decay": self.optimizer_config.transformer_weight_decay},
            {"params": self.latent_depth_motion_tokenizer.parameters(),
            "weight_decay": self.optimizer_config.transformer_weight_decay},
            {"params": self.latent_segment_motion_tokenizer.parameters(),
            "weight_decay": self.optimizer_config.transformer_weight_decay},
    ...
    def training_step(self, dataset_batch: Dict[str, Dict],) -> torch.Tensor:  # type: ignore
        ...
        cond_pixel_values = predictive_feature['state_images'][:, 0]
        target_pixel_values = predictive_feature['state_images'][:, 1]

        latent_flow_motion_tokens_up, indices_flow, commit_loss_flow = self.latent_cotracker_motion_tokenizer(
            cond_pixel_values = cond_pixel_values,
            target_pixel_values = target_pixel_values,
        )

        latent_depth_motion_tokens_up, indices_depth, commit_loss_depth = self.latent_depth_motion_tokenizer(
            cond_pixel_values = cond_pixel_values,
            target_pixel_values = target_pixel_values,
        )

        latent_semantic_motion_tokens_up, indices_semantic, commit_loss_semantic = self.latent_segment_motion_tokenizer(
            cond_pixel_values = cond_pixel_values,
            target_pixel_values = target_pixel_values,
        )

        latent_motion_tokens_up = torch.cat([latent_flow_motion_tokens_up, latent_depth_motion_tokens_up, latent_semantic_motion_tokens_up], dim=1)
        ...
        total_loss += act_loss
        total_loss += (commit_loss_flow + commit_loss_depth + commit_loss_semantic)*0.1
    ...
    def validation_step(self, dataset_batch: Dict[str, Dict]) -> Dict[
        ...
        predictive_feature['state_images'] = predictive_feature['state_images'].reshape(
            predictive_feature['state_images'].size(0), -1, predictive_feature['state_images'].size(-1)
        )
    ...
    def extract_predictive_feature(self, dataset_batch):
        ...
        frame_0 = perceptual_features[:, 0]
        frame_5 = perceptual_features[:, 0+5]
        perceptual_features = torch.stack([frame_0, frame_5], dim=1)
        perceptual_features = self.goal_emb(perceptual_features)
        time_pos_emb = (
            self.time_pos_emb.unsqueeze(0).expand(perceptual_features.size(0), -1, -1, -1)
        )
        perceptual_features = perceptual_features + time_pos_emb

        perceptual_features_for_videoformer = self.Video_Former(perceptual_features_for_videoformer)
    ...
    def diffusion_loss(
        ...
        loss, _ = self.model.loss(perceptual_emb, actions, latent_goal, noise, sigmas, latent_motion_tokens_up)
    ...
    def eval_forward(self, obs, goal):
        ...
        cond_pixel_values = perceptual_emb['state_images'][:, 0]
        target_pixel_values = perceptual_emb['state_images'][:, 1]
    ...
    def libero_eval_forward(self,image_primary,image_wrist,text):
        """
        Method for doing inference with the model.
        """
        latent_goal = self.language_goal([text])
        latent_goal = latent_goal.to(torch.float32)
        language = text

        num_frames = self.Former_num_time_embeds  # Video lenth 16
        rgb_static = image_primary.to(self.device)
        rgb_gripper = image_wrist.to(self.device)
        batch = rgb_static.shape[0]
        ...
        if self.use_original_diffusion_policy:
            act_seq = self.denoise_actions(
                torch.zeros_like(latent_goal).to(latent_goal.device),
                perceptual_emb,
                latent_goal,
            ...
            latent_cotracker_motion_tokens_up, indices_cotracker, commit_loss_cotracker = self.latent_cotracker_motion_tokenizer(
                cond_pixel_values = cond_pixel_values,
                target_pixel_values = target_pixel_values,
            )
            ...
            latent_segment_motion_tokens_up, indices_segment, commit_loss_segment = self.latent_segment_motion_tokenizer(
                cond_pixel_values = cond_pixel_values,
                target_pixel_values = target_pixel_values,
            )

            latent_motion_tokens_up = torch.cat([latent_cotracker_motion_tokens_up, latent_depth_motion_tokens_up, latent_segment_motion_tokens_up], dim=1)
    ...
    def libero_step(self,image_primary,image_wrist,text):
        """
        Do one step of inference with the model. THis method handles the action chunking case.
        Our model is trained to predict a sequence of actions.
        We only compute the sequence once every self.multistep steps.
        ...
        # 一次 rollout 10步, 机械臂执行10步之后再 rollout 下一次
        if self.rollout_step_counter % self.multistep == 0:
            pred_action_seq = self.libero_eval_forward(image_primary,image_wrist,text)
    ...
    def on_train_start(self) -> None:
        ...
        if not self.use_original_diffusion_policy:
            self.latent_cotracker_motion_tokenizer.to(dtype=self.dtype)
            self.latent_depth_motion_tokenizer.to(dtype=self.dtype)
            self.latent_segment_motion_tokenizer.to(dtype=self.dtype)

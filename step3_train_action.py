...
def load_submodule_ckpt(module, ckpt_path):
...
def train(cfg: DictConfig) -> None:
    ...
    if accelerator.is_main_process:
        ...
        experiment_dir = "icml_libero_2026"
    ...
    # chk = get_last_checkpoint(Path('/path/to/resource'))
    # Load Model
    model = hydra.utils.instantiate(cfg.model)

    if cfg.ckpt_path_flow:
        load_submodule_ckpt(
            model.latent_flow_motion_tokenizer,
            cfg.ckpt_path_flow
        )

    if cfg.ckpt_path_depth:
        load_submodule_ckpt(
            model.latent_depth_motion_tokenizer,
            cfg.ckpt_path_depth
        )

    if cfg.ckpt_path_semantic:
        load_submodule_ckpt(
            model.latent_semantic_motion_tokenizer,
            cfg.ckpt_path_semantic
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--token_ckpt_path_flow", type=str, default="")
    parser.add_argument("--token_ckpt_path_depth", type=str, default="")
    parser.add_argument("--token_ckpt_path_semantic", type=str, default="")
    parser.add_argument("--use_half_data", type=bool, default=False)
    ...
    if args.token_ckpt_path_flow:
        cfg.ckpt_path_flow = args.token_ckpt_path_flow
        print(f"[INFO] Overriding cfg.ckpt_path with args.token_ckpt_path_flow: {cfg.ckpt_path_flow}")
    if args.token_ckpt_path_depth:
        cfg.ckpt_path_depth = args.token_ckpt_path_depth
        print(f"[INFO] Overriding cfg.ckpt_path with args.token_ckpt_path_depth: {cfg.ckpt_path_depth}")
    if args.token_ckpt_path_semantic:
        cfg.ckpt_path_semantic = args.token_ckpt_path_semantic
        print(f"[INFO] Overriding cfg.ckpt_path with args.token_ckpt_path_semantic: {cfg.ckpt_path_semantic}")
    if args.use_half_data is True:
        cfg.datamodule.use_half_data = True
        print(">>>>>>>>>>>> Training on half dataset! <<<<<<<<<<<<")
    train(cfg)
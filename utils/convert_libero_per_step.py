...
def main(rank, port, num_worker, start_episode_idx=0, end_episode_idx=None, dataset_name_arg="libero_90", src_dir_arg=None, tgt_dir_arg=None):
    if num_worker > 1:
        setup(rank, world_size=num_worker, port=port)

    global dataset_name
    dataset_name = dataset_name_arg
    src_dir = src_dir_arg or os.getenv("LIBERO_SOURCE_DIR", f"/path/to/LIBERO-datasets/{dataset_name}")
    tgt_dir = Path(tgt_dir_arg or os.getenv("LIBERO_CONVERTED_DIR", f"outputs/{dataset_name}_converted"))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_name", default="libero_90")
    parser.add_argument("--src_dir", default=None)
    parser.add_argument("--tgt_dir", default=None)
    parser.add_argument("--start_episode_idx", type=int, default=0)
    parser.add_argument("--end_episode_idx", type=int, default=None)
    ...
    mp.spawn(
        main,
        args=(port, num_worker, start_episode_idx, end_episode_idx, args.dataset_name, args.src_dir, args.tgt_dir),
        nprocs=num_worker,
        join=True,
    )
